...
def comp(function, list_result, cc):
    code = compile(function, '<string>', "exec")
    loc = {}
    glob = {'time_measur': time_measur, 'queue': list_result, 'cc': cc}
    exec(code, glob, loc)
    test = loc["_test_func"]
...
"""
    ...
    def compute(self, timeout):
        ...
        proc_args = (self.function, list_result, self)
        ...
        # Logarithmic equation  y = a * log(x) +b
        ...
        NR2 = np.polyfit(y, x, 2)

        def how_many_elements(x):
            return np.polyval(NR2, [x])

        need_time = how_much_time
        max_elem = how_many_elements
        ...
        if diff == O2 and O2 / O1 > delta:
            ...
            NR1 = np.polyfit(y, x, 1)

            def how_many_elements(x):
                return np.polyval(NR1, [x])
        ...
        if diff == O1 and O1 / Olog > delta:
            ...
            NRlog = np.polyfit(x, np.log(y), 2)

            def how_many_elements(x):
                return np.polyval(NRlog, [x])
        ...
        if diff == Olog and Olog / O0 > delta:
            ...
            NR0 = np.polyfit(y, x, 0)

            def how_many_elements(x):
                return np.polyval(NR0, [x])
        ...
        logger.info(answer)
        plt.show()
        return (answer, need_time, max_elem)
